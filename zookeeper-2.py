...
try:
	...
	def brokerthread(conn, addr):
		while True:
			...
			metalog = open("meta.log", "a")
			metalog.write(":".join([topic, partition, offset]))
			metalog.write("\n")
			metalog.close()
	
	port = 3750
	s1.bind(('localhost', 3750))
	s1.listen(100)
	...
	while i<3:
		...
		broker_port = int(conn.recv(2048).decode())
	...
	def clientthread(conn, addr,list_of_clients, topic):
		...
		while True:
			try:
				...
				partition = message[0] % partitions
				port = get_broker(partition, topic)
		...
		try:
			...
			if(msg == "producer"):
				...
				for i in topics[topic]:
					conn.sendmsg([str(i[0]).encode(), "\n".encode(), str(i[1]).encode(), "\n".encode()])
					sleep(1)
				cons_f = open("meta.log", "rb")
				cons_f.seek(-2, os.SEEK_END)
				while cons_f.read(1) != b'\n':
				    	cons_f.seek(-2, os.SEEK_CUR)
				
				line = cons_f.readline().decode().strip().split(":")
				print(line[2])
				conn.send(line[2].encode())
